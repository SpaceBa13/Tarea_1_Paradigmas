...
PRINT_RESULT:
    ; Imprimir la parte entera (32 bits)
    MOV DX, perimetro_ent_alta   ; Parte alta del entero
    MOV AX, perimetro_ent_baja   ; Parte baja del entero
    CALL PRINT_DWORD
    ...
    ; Imprimir la parte decimal (16 bits)
    MOV AX, perimetro_dec_alta   ; Parte alta del decimal
    CALL PRINT_WORD
    MOV AX, perimetro_dec_baja   ; Parte baja del decimal
    CALL PRINT_WORD
    ...
    ; Imprimir el área (32 bits), similar a la parte anterior
...
PRINT_DWORD:
    ...
    NEG DX
    NEG AX
    ; Ajusta el signo negativo en el número
    
POSITIVE_DWORD:
    ; Dividir DX:AX entre 10 y convertir a ASCII
    MOV BX, 10
    MOV CX, 0
    XOR DX, DX
    
    ; Dividir el número (DX:AX) en dígitos y almacenarlos en la pila
    DIVIDE_DWORD_LOOP:
        XOR DX, DX
        DIV BX
        PUSH DX
        INC CX
        TEST AX, AX
        JNZ DIVIDE_DWORD_LOOP

    ; Asegúrate de imprimir al menos un dígito
    CMP CX, 0
    JZ PRINT_ZERO
    
    ; Imprimir los dígitos almacenados en la pila
    PRINT_DWORD_DIGITS:
        POP DX
        ADD DL, '0'
        MOV AH, 02h
        INT 21h
        LOOP PRINT_DWORD_DIGITS
...
PRINT_WORD:
    ; Convierte un número en AX a ASCII y lo imprime (0-9999)
    
    ; Usamos una pila para imprimir dígitos de forma correcta
    MOV CX, 0          ; Inicializa el contador de dígitos
    MOV BX, 10
    
    ; Asegúrate de que al menos un dígito sea procesado
    CMP AX, 0
    JZ PRINT_ZERO
