...
.data            
    ...
    blank_char db ''  ; Carï¿½cter espacio para usar en la funciï¿½n de retroceso
...
READ_LOOP:
    mov ah, 01h          ; Funciï¿½n de lectura de un carï¿½cter
    int 21h              ; Leer carï¿½cter del teclado

    CMP AL, 0DH          ; Verificar si el carï¿½cter es Enter (fin de entrada)
    JE DONE              ; Si es Enter, terminar lectura

    CMP AL, 08H          ; Verificar si el carï¿½cter es Backspace
    JE BACKSPACE         ; Si es Backspace, borrar el ï¿½ltimo carï¿½cter

    CMP AL, '.'          ; Verificar si el carï¿½cter es un punto decimal
    JE DECIMAL_POINT     ; Si es punto decimal, cambiar a modo decimal

    ; Convertir carï¿½cter ASCII a nï¿½mero (0-9)
    SUB AL, '0'
    CMP AL, 9
    JA INVALID_CHAR      ; Verificar si el carï¿½cter es un dï¿½gito vï¿½lido

    ; Guardar carï¿½cter en buffer
    MOV [DI], AL
    INC DI
    INC CX
...
DECIMAL_POINT:
    ...
    CMP BL, 1
    JE INVALID_CHAR      ; Si ya hay un punto decimal, es invï¿½lido
...
BACKSPACE:
    ...
    ; Mover cursor hacia atrï¿½s y sobrescribir el carï¿½cter con un espacio
    MOV AH, 02H          ; Funciï¿½n para mover el cursor
    MOV DL, 08H          ; Mover el cursor una posiciï¿½n hacia la izquierda
    INT 21H              ; Ejecutar
    
    MOV AH, 09H          ; Funciï¿½n para mostrar caracteres
    LEA DX, blank_char   ; Direcciï¿½n del carï¿½cter espacio
    INT 21H              ; Mostrar el espacio en blanco

    ; Mover el cursor hacia atrï¿½s de nuevo para que quede en la posiciï¿½n correcta
    MOV AH, 02H          ; Funciï¿½n para mover el cursor
    MOV DL, 08H          ; Mover el cursor una posiciï¿½n hacia la izquierda
    INT 21H              ; Ejecutar

    ; Verificar si el carï¿½cter a borrar es un punto decimal
    MOV AL, [DI]         ; Cargar el carï¿½cter que se va a borrar
    CMP AL, '.'          
...
INVALID_CHAR:
    ; Eliminar el ï¿½ltimo carï¿½cter ingresado (simular backspace)
...
STR_TO_INT:
    xor ax, ax            ; Limpiar AX para usarlo como el nï¿½mero entero
    mov cx, 10            ; Base 10 para la conversiï¿½n 
    
convert_loop:
    mov bl, [si]          ; Cargar el siguiente carï¿½cter
    ...
    inc si                ; Mover al siguiente carï¿½cter
    jmp convert_loop      ; Continuar con el siguiente carï¿½cter
...
PRINT_DWORD:
    ; Convierte un número de 32 bits (DX:AX) a ASCII y lo imprime
...
PRINT_DIGITS_32:
    ...
    INT 21h
    LOOP PRINT_DIGITS_32  ; Repetir hasta que se impriman todos los dígitos
